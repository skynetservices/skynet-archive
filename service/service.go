...
func CreateService(sd ServiceDelegate, c *skynet.ServiceConfig) (s *Service) {
	...
	log.Printf(log.TRACE, "%+v", skynet.ServiceCreated{
	...
	log.Printf(log.TRACE, "%+v", RegisteredMethods{rpcForwarder.MethodNames})
...
func (s *Service) listen(addr *skynet.BindAddr, bindWait *sync.WaitGroup) {
	...
	log.Printf(log.TRACE, "%+v", ServiceListening{
		Addr:          addr,
		ServiceConfig: s.Config,
	})
...
func (s *Service) doozer() *skynet.DoozerConnection {
	if s.DoozerConn == nil {
		s.DoozerConn = skynet.NewDoozerConnectionFromConfig(*s.Config.DoozerConfig)
...
func (s *Service) Start(register bool) (done *sync.WaitGroup) {
	...
	if s.Config.AdminAddr != nil {
		...
		log.Printf(log.TRACE, "%+v", AdminNotListening{s.Config})
	}
...
func (s *Service) cleanupDoozerEntriesForAddr(addr *skynet.BindAddr) {
	...
	for _, i := range instances {
		log.Println(log.TRACE, "Cleaning up old doozer entry with conflicting addr "+
...
func (s *Service) register() {
	...
	s.Registered = true
	log.Printf(log.TRACE, "%+v", ServiceRegistered{s.Config})
	s.UpdateDoozerServiceInfo()
...
func (s *Service) unregister() {
	...
	s.Registered = false
	log.Printf(log.TRACE, "%+v", ServiceUnregistered{s.Config})
	s.UpdateDoozerServiceInfo()
	...
	for {
		...
		case sig := <-c:
			...
			case syscall.SIGINT, syscall.SIGKILL, syscall.SIGQUIT,
				...
				log.Printf(log.TRACE, "%+v", KillSignal{sig.(syscall.Signal)})
				s.Shutdown()
			}
		}
	}
